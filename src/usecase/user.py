...
class UserService():
    ...
    def wants_to_create_user(self, user_data: UserSignUpDTO):
        ...
        try:
            ...
            if self.user_repository.find_by_email(user_data.email) is not None:
                raise exceptions.HTTPException(status_code=409, detail="Email already exists") from exc
            else:
                raise exceptions.HTTPException(status_code=409, detail="Username already exists") from exc
        else:
            try:
                self.auth_service.sign_up(user_data.email, user_data.password)
            except Exception as exc:
                self.user_repository.delete(user_data.username)
                raise exceptions.HTTPException(status_code=500, detail="Firebase Error") from exc
    ...
    def wants_to_delete_user(self, username: str):
        ...
        try:
            ...
            raise exceptions.HTTPException(status_code=404, detail="user to delete not found") from exc
    ...
    def wants_to_update_user(self, user_data: UserDTO):
        ...
        try:
            ...
            raise exceptions.HTTPException(status_code=404, detail="user to update not found") from exc
